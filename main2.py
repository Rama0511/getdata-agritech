...
def run_datamic_script():
    current_date = datetime.now().replace(hour=23, minute=59, second=0).strftime('%Y-%m-%d %H:%M:%S')
    start_date = datetime.now() - timedelta(days=3)
    start_date = start_date.replace(hour=0, minute=0, second=0).strftime('%Y-%m-%d %H:%M:%S')
    try:
        ...
        for sensor_company_id, sensor_company_name in sensor_companies:
            ...
            if api_response and 'data' in api_response and 'data' in api_response['data']:
                ...
                labels = pd.date_range(start=start_date, end=current_date, freq='H').strftime('%Y-%m-%d %H:%M:%S').tolist()
                values = [0] * len(labels)
